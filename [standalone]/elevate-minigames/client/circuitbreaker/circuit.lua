-- shizzle er nuppet fra github vern
...
local defaultDelayStartTimeMs <const> = 2500
local minDelayEndGameTimeMs <const> = 2500
local maxDelayEndGameTimeMs <const> = 2500
...
local function runMinigameTask(levelNumber, difficultyLevel, cursorSpeed, delayStartMs, minFailureDelayTimeMs, maxFailureDelayTimeMs, disconnectChance, disconnectCheckRateMs, minReconnectTimeMs, maxReconnectTimeMs)
    ...
    while true do
        ...
        if GetGameTimer() - delayedStartTime >= 0 and not _isEndScreenActive and _cursor.isAlive then
...
---@param difficultyLevel Difficulty 0 - 3, take a look at globals.lua
---@return GameStatus
local function runDefaultMiniGameFromDifficulty(difficultyLevel)
...
exports('CircuitBreaker', runDefaultMiniGameFromDifficulty)