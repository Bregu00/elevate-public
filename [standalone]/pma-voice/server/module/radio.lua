...
function addPlayerToRadio(source, radioChannel)
	...
	logger.verbose('[radio] Added %s to radio %s', source, radioChannel)
    	...
    	local xPlayer = ESX.GetPlayerFromId(source) -- Get the ESX player object
    
    if xPlayer then
        local playerName = xPlayer.getName()
        local playerJob = xPlayer.job.name
		exports.onl_logsender:SendLog(source, playerName .. " (" .. playerJob .. ") joined radio " .. radioChannel, {
            labels = {
                job = "logs",
				serverId = true,
                discordId = true,
            ...
            discordTitle = "Joined radio" .. radioChannel,
            discordWebhook = "https://discord.com/api/webhooks/1341929248436387892/CX6CfRsHIQwRaEz5NIM-71ffqsLSQEE_qpTrREYS6MJfx3hHesavMu07Le0cGfsh0dHI?thread_id=1341929130333311016" -- Another webhook
        })
