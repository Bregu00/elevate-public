...
function splitOnce(str, delimiter)
    local part1, part2 = str:match("([^" .. delimiter .. "]+)" .. delimiter .. "(.*)")
    return part1, part2
end

local function getAllHardwareIds(source)
    local tokens = {}
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    local _, discordID = splitOnce(getIdentifier("discord") or nil, ":")
    local _, steamID = splitOnce(getIdentifier("steam") or nil, ":")
    if not steamID or not discordID then
        deferrals.done("SteamID og DiscordID er ikke fundet")
        CancelEvent()
        return
    end
    Config.CheckBanned(name, steamID, discordID, json.encode(getAllHardwareIds(src)), function(success, decoded)
        if success then
            if decoded then
                if decoded.banned then
                    ...
                    =====================================
                    🚫 Du er banned fra ElevateRP! 🚫
                    =====================================
                    🔹 Grund: %s
                    🔹 Oprettet: %s
                    🔹 Udløber: %s
                    
                    💡 For at appellere, opret en ticket:
                    🔗 discord.gg/elevaterp
                    =====================================
                    
                    ]]):format(
                        decoded.ban.title or "Ingen grund angivet",
                        decoded.ban.timestamp and os.date("%Y-%m-%d %H:%M:%S", decoded.ban.timestamp + 7200) or "Ukendt",
                        decoded.ban.expires_at and os.date("%Y-%m-%d %H:%M:%S", decoded.ban.expires_at + 7200) or "Aldrig"
                    )
            ...
            deferrals.done("Der opstod en fejl under forbindelsen til serveren. Prøv igen senere.")
        end
    end)
