...
local function openInventory(source, invType, data, ignoreSecurityChecks)
	...
	local bagAdd = lib.callback.await('jungurum-smallresources:client:getBagSize', source)
	if bagAdd then
		left.maxWeight = shared.playerweight + bagAdd
	else
		left.maxWeight = shared.playerweight
	end
...
lib.addCommand('allitemsinv', {
    ...
    local allItems = require 'data.items'
    local stashId = args.stashId

    local stash = Inventory.Create(stashId, "Alle Items", "stash", 10000, 0, 100000)

    if stash then
        for itemName, itemData in pairs(allItems) do
            Inventory.AddItem(stashId, itemName, 1)
        end

        Inventory.InspectInventory(source, stashId)
    else
    end
end)