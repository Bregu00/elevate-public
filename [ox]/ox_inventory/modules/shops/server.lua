...
local function canAffordItem(inv, currency, price, source)
	...
	if currency == 'auto' or currency == 'autoall' then
		if not xPlayer then
            print(('[ERROR] Player with ID %s not found.'):format(source))
            return false
        end
		if currency == 'autoall' and xPlayer.getAccount('black_money').money >= price then
			return true, 'black_money'
		elseif xPlayer.getAccount('money').money >= price then
			return true, 'money'
		elseif xPlayer.getAccount('bank').money >= price then
			if xPlayer.get("isAccountLocked") then
				TriggerClientEvent('ox_lib:notify', source, {
					type = 'error',
					description = 'Din konto er låst. Domstolen har lukket din konto.',
					duration = 10000,
				})
				return false
			end
...
local function removeCurrency(inv, currency, price, source, newCurrency)
	local xPlayer = ESX.GetPlayerFromId(source)
	if currency == 'auto' or currency == 'autoall' then
		if xPlayer then
            xPlayer.removeAccountMoney(newCurrency, price)
        end
		return
    end
	...
	if data.toType == 'player' then
		...
		if fromData then
			...
			if toData == nil or (fromItem.name == toItem?.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
				...
				local canAfford, newCurrency = canAffordItem(playerInv, currency, price, source)
				...
				removeCurrency(playerInv, currency, price, source, newCurrency)
				...
				local itemCurrency = (currency ~= 'money' and currency ~= 'bank') and Items(currency)
				if not itemCurrency and currency ~= 'money' and currency ~= 'bank' then
					print(("[ERROR] Invalid currency: %s"):format(currency or "nil"))
					return false, false, { type = 'error', description = locale('invalid_currency') }
				end

				local currencySymbol = (currency == 'money' and locale('$')) or (currency == 'bank' and locale('$')) or math.groupdigits(price)
				local currencyLabel = (currency == 'money' or currency == 'bank') and math.groupdigits(price) or (' ' .. (itemCurrency and itemCurrency.label or ""))
