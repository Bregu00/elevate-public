...
function Utils.IsValidHolster(type)
	local playerModel = GetEntityModel(cache.ped)
	if not Holsters.ValidPedHolsters[playerModel] then
		return false
	end

	local holsterType = ""
	if type == "remove" then holsterType = "holsterRemoveWeapon" end
	if type == "add" then holsterType = "holsterAddWeapon" end

	if holsterType == "" then
		return false
	end

	local holsters = Holsters.ValidPedHolsters[playerModel]
	for variation, holster in pairs(holsters) do
		if holster[holsterType] then
			local currentcomponent = GetPedDrawableVariation(cache.ped, variation)
			if holster[holsterType][currentcomponent] then
				return true, variation, currentcomponent
			end
		end
	end
...
function Utils.HandleWeaponHolster(variation, component, typeholster)
	local playerModel = GetEntityModel(cache.ped)
	if not Holsters.ValidPedHolsters[playerModel] then
		return
	end
	...
	local holsters = Holsters.ValidPedHolsters[playerModel]
	if not holsters[variation] then
		return
	end

	local holsterType = ""
	if typeholster == "remove" then holsterType = "holsterRemoveWeapon" end
	if typeholster == "add" then holsterType = "holsterAddWeapon" end

	if holsterType == "" then
		return
	end

	if not holsters[variation][holsterType] then
		return
	end

	if not holsters[variation][holsterType][component] then
		return
	end

	local holster = holsters[variation][holsterType][component]
	if type(holster) == "boolean" then
		return
	end
	
	if not holster.newVariation and not holster.newComponent then
		return
	end

	SetPedComponentVariation(cache.ped, holster.newVariation, holster.newComponent, 0, 0)
end
