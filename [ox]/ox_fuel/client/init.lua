...
CreateThread(function()
	...
	SetInterval(function()
		for i = 1, #config.Locations do
			...
			if distance <= 100 and not DoesEntityExist(models[i]) then
				models[i] = CreateObject(GetHashKey('electric_charger'), coords.x, coords.y, coords.z, false, false, false)

				while not DoesEntityExist(models[i]) do
					Wait(100)
				end
		
				SetEntityHeading(models[i], coords.w - 180)
				FreezeEntityPosition(models[i], true)
			elseif distance > 100 and DoesEntityExist(models[i]) then
