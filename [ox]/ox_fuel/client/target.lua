	...
	exports.ox_target:addModel(config.pumpModels, {
		{
			...
			canInteract = function(entity)
				if state.isFueling or cache.vehicle or lib.progressActive() or config.electricVehicles[GetEntityModel(state.lastVehicle)] then
					return false
				end
	...
	exports.ox_target:addModel(config.pumpModels, {
		{
			...
			label = locale('start_fueling'),
			canInteract = function(entity)
				if state.isFueling or cache.vehicle or not DoesVehicleUseFuel(state.lastVehicle) or config.electricVehicles[GetEntityModel(state.lastVehicle)] then
					return false
				end
	...
	exports.ox_target:addGlobalVehicle({
		{
			...
			label = locale('start_fueling'),
			canInteract = function(entity)
				if state.isFueling or cache.vehicle or lib.progressActive() or not DoesVehicleUseFuel(entity) or config.electricVehicles[GetEntityModel(state.lastVehicle)] then
					return false
				end
...
exports['ox_target']:addModel(config.chargerModels, {
	{
		...
		onSelect = function()
			if utils.getMoney() >= config.priceTick then
				...
				lib.notify({ type = 'error', description = locale('recharge_cannot_afford') })
			end
		end,
		...
		canInteract = function(entity)
			...
			return state.lastVehicle and #(GetEntityCoords(state.lastVehicle) - GetEntityCoords(cache.ped)) <= 3 and config.electricVehicles[GetEntityModel(state.lastVehicle)]
		end
	},
})