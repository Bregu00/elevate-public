...
RegisterNetEvent("jg-dealerships:server:purchase-vehicle-pre-check", function(src, plate, model, dealershipId, purchaseType, amountToPay, paymentMethod, societyType, society, financed, directSale, sellerPlayerId, noOfPayments, downPayment, cb)
  ...
  if directSale == nil and amountToPay > 250000 then
    local carDealerOnline = exports["jungurum-lib"]:getJobCount("pdm") > 0

    if carDealerOnline then
      allowed = false
    end
  end

  if not allowed then
    TriggerClientEvent('esx:showNotification', src, "Fejl, er der en forhandler på arbejde?", "error")
    return cb(false)
  end
