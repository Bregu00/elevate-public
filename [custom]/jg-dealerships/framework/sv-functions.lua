...
function Framework.Server.IsAdmin(src)
  ...
  return admins[xPlayer.group] or false
end
...
function Framework.Server.GetSocietyBalance(society, societyType)
  ...
  elseif Config.Framework == "ESX" then
    local _, pos = string.find(society, "_") -- Find the position of the underscore
    local job = string.sub(society, pos + 1) -- Get substring after the underscore
...
function Framework.Server.RemoveFromSocietyFund(societyName, societyType, amount)
  ...
  elseif Config.Framework == "ESX" then
    exports['elevate-multijob']:removeAccountBalance(societyName, amount)
  end
end
...
function Framework.Server.SaveVehicleToGarage(src, purchaseType, society, societyType, model, plate, financed, financeData, dealershipData)
  local vehicleId = nil
  ...
  elseif Config.Framework == "ESX" then
    ...
    local insertType = nil
    if dealershipData.isBoat then 
      insertType = 'boat'
    else 
      insertType = 'car'
    end
    ...
    if purchaseType == "society" and societyType == "job" then
      vehicleId = MySQL.insert.await(
        "INSERT INTO owned_vehicles (owner, plate, vehicle, stored, financed, finance_data, job_vehicle, job_vehicle_rank, type) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)",
        {society, plate, props, "global", financed, json.encode(financeData), 1, 0, insertType}
      )
    else
      vehicleId = MySQL.insert.await(
        "INSERT INTO owned_vehicles (owner, plate, vehicle, stored, financed, finance_data, type) VALUES(?, ?, ?, ?, ?, ?, ?)",
        {identifier, plate, props, "global", financed, json.encode(financeData), insertType}
      )
    end
  end
