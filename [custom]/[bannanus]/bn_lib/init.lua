...
-- Initialize export from 'bn_lib' resource
local export = exports[bn_lib]
...
local loadModule = function(self, module)
    ...
    local chunk = LoadResourceFile(bn_lib, ('%s/%s.lua'):format(dir, context))
    local shared = LoadResourceFile(bn_lib, ('%s/shared.lua'):format(dir))
    ...
    if chunk then
        ...
        local fn, err = load(chunk, ('@@bn_lib/modules/%s/%s.lua'):format(module, context), 't', env)
...
-- Define the BN table with metatable for call and index operations
local BN = setmetatable({
    name = bn_lib,
    context = context,
    onCache = function(key, cb)
        AddEventHandler(('bn_lib:cache:%s'):format(key), cb)
    end
}, {
    __index = call,
    __call = call,
...
-- Set the global BN environment
_ENV.BN = BN

-- Load all modules mentioned in the resource metadata
for i = 1, GetNumResourceMetadata(bn_lib, bn_lib) do
    local name = GetResourceMetadata(bn_lib, bn_lib, i - 1)

    if not rawget(BN, name) then
        local module = loadModule(BN, name)
