...
BN.RegisterCommand = function(command, permission, callback, help, params)
    ...
    local commandHandler = function(source, args, rawCommand)
        ...
        if not parsedArgs then
            TriggerClientEvent('bn_bridge:notification', source, ("Invalid command arguments for command '%s'. Expected: %s"):format(command, json.encode(params)), 'error')
            return
        end
        ...
        if not success then
            ...
            TriggerClientEvent('bn_bridge:notification', source, ("Command '%s' failed to execute."):format(command), 'error')
        end
    end
