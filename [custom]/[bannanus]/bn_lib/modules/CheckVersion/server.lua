...
BN.CheckVersion = function(repo)
    ...
    SetTimeout(1000, function()
        ...
        PerformHttpRequest(url, function(status, response)
            ...
            local cv = {table.unpack(BN.String.Split(currentVersion, '%.'))}
            local lv = {table.unpack(BN.String.Split(latestVersion, '%.'))}
