...
local sendQueue = function(name)
    if #logQueue[name] > 0 then
        local postData = {username = 'BN Logs', embeds = {}}
...
--- Public interface for logging, exposed through the BN namespace
...
BN.Log = function(name, title, color, message, tagEveryone)
