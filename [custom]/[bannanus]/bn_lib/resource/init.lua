...
-- Initialization of 'bn_lib' and context determination
local bn_lib = 'bn_lib'
local context = IsDuplicityVersion() and 'server' or 'client'

-- BN Table with metatable for handling module loading and framework integration
BN = setmetatable({
    name = bn_lib,
    context = context,
}, {
    ...
    __index = function(self, key)
        ...
        if chunk then
            ...
            local fn, err = load(chunk, ('@@bn_lib/%s/%s.lua'):format(key, self.context), 't', env)
