...
lib.callback.register("visualz_selldrugs:sellDrugs", function(source, networkId, drug, zone)
  ...
  if Entity(entity).state["hasSold_" .. source] then
    return { type = "error", description = Config.Notify["AlreadySoldToThisNPC"]() }
  end

  Entity(entity).state:set("hasSold_" .. source, true, true)
  ...
  local copAmount = exports['jungurum-lib']:getJobCount('police')

  local totalPrice = (price * amount) * (Config.copMultiplier[copAmount] or Config.copMultiplier[#Config.copMultiplier])
  ...
  SendLog(Logs["SoldDrugs"], 2829617, "Solgt stoffer", discordMessage, " Elevate " .. os.date("%d/%m/%Y %H:%M:%S"))

  SellDrugEvent(source, drug, price, amount, zone)

  return { type = "info", description = Config.Notify["AcceptNotify"](drug, amount, totalPrice) }
end)
...
function CallPolice(xPlayer, zone, drug)
  ...
  exports['tk_dispatch']:addCall({
      title = 'Mistænkelig Salg',
      code = 'Stofsalg',
      priority = 'Prioritet 1',
      message = 'En person er blevet spottet sælge noget mistænkligt',
      coords = coords,
      showTime = 7500, -- will be shown on screen (as notification) for 10 seconds
      removeTime = 1000 * 60 * 2, -- will be removed after 10 minutes
      color = 'green',
      flash = true,
      playSound = false,
      blip = {
  ...
  TriggerEvent('elevate_sheets:logtosheet', 'Stof Salg', exports["visualz_zones"]:getZoneLabel(zone))
end
