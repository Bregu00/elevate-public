...
local allFormattedHouses = {}


CreateThread(function()
    local allHouses = MySQL.Sync.fetchAll('SELECT * FROM `player_houses`')
    for k, v in pairs(allHouses) do
        v.keyholders = json.decode(v.keyholders) or {}
        allFormattedHouses[v.owner] = v
    end
end)

RegisterNetEvent('housing:giveKeys', function(playerSrc, houseName)
    print(("Player %d was given keys to house: %s"):format(playerSrc, houseName))
    -- Example: Log to console
    print("Keys granted event fired for player:", playerSrc, "House:", houseName)
end)

RegisterNetEvent('housing:removeKeys', function(playerSrc, houseName)
    print(("Player %d lost keys to house: %s"):format(playerSrc, houseName))
    -- Example: Log to console
    print("Keys removed event fired for player:", playerSrc, "House:", houseName)
end)
...
lib.callback.register("housing:getKeyHolders", function(source, house)
    local result = MySQL.Sync.fetchAll('SELECT `keyholders` FROM `player_houses` WHERE `house` = ?', { house })
    if result[1] then
        return json.decode(result[1].keyholders)
    end
    return {}
end)

lib.callback.register("houses:server:getAllHouses", function(source)
    local result = MySQL.Sync.fetchAll('SELECT * FROM `houselocations`')
    return result
end)

lib.callback.register("houses:server:getHouseInfo", function(source, houseName)
    local result = MySQL.Sync.fetchAll('SELECT * FROM `player_houses` WHERE `house` = ?', { houseName })
    if result[1] then
        return result[1]
    end
end)

lib.callback.register("housing:server:getOwnerInfo", function(source, identifier)
    local result = MySQL.Sync.fetchAll('SELECT * FROM `users` WHERE `identifier` = ?', { identifier })
    if result[1] then
        return result[1]
    end
...
lib.callback.register("housing:server:getPhoneNumber", function(source, identifier)
    local phoneNumber = exports["lb-phone"]:GetEquippedPhoneNumber(identifier)
    return phoneNumber
end)


lib.callback.register("houses:server:getHouseBlipInfo", function(source, license)
    local result = MySQL.query.await([[
        SELECT ph.id, ph.house, hl.coords
        FROM player_houses ph
        JOIN houselocations hl ON hl.name = ph.house
        WHERE JSON_CONTAINS(ph.keyholders, @license, "$")
        AND ph.owner != @owner
    ]], {
        ['@license'] = json.encode(license),
        ['@owner'] = license
    })
