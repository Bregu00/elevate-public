            ...
            for _, strPlayerId in pairs(GetPlayers()) do
                ...
                if Player then
                    ...
                    table.insert(PlayerList, {
                        id = playerId, name = GetPlayerName(playerId), identifiers = json.encode(identifiers), role = LoadedRole[playerId], bank = '$' .. comma_value(Player.getAccount('bank').money), cash = '$' .. comma_value(Player.getMoney()),
                        ...
                        lastonline = lastOnlineResult[1]?.last_seen or 'Unknown', amountofVehicles = amountofVehicles[1]?.count or 0, job = Player.job, rank = Player.job.grade_label, health = GetEntityHealth(GetPlayerPed(Player.source)) - 100,
            ...
            for k, v in pairs(results) do
                if v.accounts then
                    ...
                    for _, v1 in pairs(results2) do
                        if results[k] == nil then return end
                        vehicles[#vehicles + 1] = { citizenid = v.identifier, firstname = v.firstname, lastname = v.lastname, vehicle = v1.vehicle }
                    end
                end
            end
