...
RegisterServerEvent('919-admin:server:KillPlayer', function(playerId)
    ...
    if AdminPanel.HasPermission(src, 'kill') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'KICK', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** ' .. Lang:t('alerts.killed') .. ' **' .. GetPlayerName(playerId) .. '** [' .. playerId .. ']', false)
    end
end)
...
RegisterServerEvent('919-admin:server:SavePlayer', function(playerId)
    ...
    if AdminPanel.HasPermission(src, 'savedata') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'KICK', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '**' .. Lang:t('alerts.savedToDB', { value = GetPlayerName(playerId) }) .. '** [' .. playerId .. ']', false)
    end
end)
...
RegisterServerEvent('919-admin:server:RepairPlayerVehicle', function(playerId)
    ...
    if AdminPanel.HasPermission(src, 'repairplayervehicle') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'KICK', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** ' .. Lang:t('alerts.repairedPlayerVehicle') .. '** [' .. playerId .. ']', false)
...
RegisterServerEvent('919-admin:server:KickPlayer', function(playerId, reason)
    ...
    if AdminPanel.HasPermission(src, 'kick') then
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'KICK', 'red', Lang:t('alerts.kickedPlayer', { value = GetPlayerName(src), value2 = GetPlayerName(playerId), value3 = playerId, value4 = reason }), false)
...
RegisterServerEvent('919-admin:server:Freeze', function(playerId)
    ...
    if AdminPanel.HasPermission(src, 'freeze') then
        if not Frozen[playerId] then
            ...
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Frozen', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** ' .. Lang:t('alerts.froze') .. ' **' .. GetPlayerName(playerId) .. '** [' .. playerId .. ']', false)
        else
            ...
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Frozen', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** ' .. Lang:t('alerts.unfroze') .. ' **' .. GetPlayerName(playerId) .. '** [' .. playerId .. ']', false)
        end
    end
end)
        ...
        if player ~= 'OFFLINE' then
            ...
            if citizenid ~= nil then
                ...
                if Config.Framework == 'qbcore' or Config.Framework == 'qbox' then 
                    MySQL.query.await('SELECT * FROM `' .. Config.DB.CharactersTable .. '` WHERE `citizenid` = ?', { citizenid })
                else
                    MySQL.query.await('SELECT * FROM `' .. Config.DB.CharactersTable .. '` WHERE `identifier` = ?', { citizenid })
                end
                if result[1] then
                    local online = false
...
RegisterNetEvent('919-admin:server:CuffPlayer', function(target)
    ...
    if AdminPanel.HasPermission(src, 'cuff') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Cuff Player', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** cuffed/uncuffed **' .. GetPlayerName(target) .. '** [' .. target .. ']', false)
        TriggerClientEvent("ND_Police:client:setCuffed", target, true, "back", "cuff")
...
RegisterServerEvent('919-admin:server:RevivePlayer', function(target)
    ...
    if AdminPanel.HasPermission(src, 'revive') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Revive', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** revived **' .. GetPlayerName(target) .. '** [' .. target .. ']', false)
...
RegisterServerEvent('919-admin:server:ReviveAll', function()
    ...
    if AdminPanel.HasPermission(src, 'reviveall') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Revive', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** revived all players.', false)
...
RegisterServerEvent('919-admin:server:FeedPlayer', function(target)
    ...
    if AdminPanel.HasPermission(src, 'foodandwater') then
        ...
        elseif Config.Framework == 'esx' then
            TriggerClientEvent('esx_status:add', target, "hunger", 1000000)
            TriggerClientEvent('esx_status:add', target, "thirst", 1000000)
        end
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Food & Water Max', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** fed and watered **' .. GetPlayerName(target) .. '** [' .. target .. ']', false)
    end
end)
...
RegisterServerEvent('919-admin:server:RelieveStress', function(target)
    ...
    if AdminPanel.HasPermission(src, 'relievestress') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Relieve Stress', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** relieved stress of **' .. GetPlayerName(target) .. '** [' .. target .. ']', false)
    end
end)
...
RegisterNetEvent('919-admin:server:RequestSpectate', function(player)
    ...
    if AdminPanel.HasPermission(src, 'spectate') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Spectate', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** started spectating **' .. GetPlayerName(player) .. '** [' .. player .. ']', false)
    end
end)
        ...
        repeat
            ...
            if Player ~= nil then
                if i == src then
                    ...
                    TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Spectate', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** started spectating **' .. GetPlayerName(i) .. '** [' .. i .. ']', false)
...
RegisterNetEvent('919-admin:server:SaveCar', function(mods, modelname, hash, plate, senderId)
    ...
    if result[1] == nil then
        ...
        if senderId then
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Admin Car', 'red', '**STAFF MEMBER ' .. GetPlayerName(senderId) .. '** has added a ' .. modelname .. ' (' .. plate .. ') to the garage of ' .. GetPlayerName(src), false)
            ...
            if result[1] then
                MySQL.query('DELETE FROM `' .. Config.DB.CharactersTable .. '` WHERE citizenid = ? LIMIT 1', { citizenId }, function(rowsAffected)
                    if rowsAffected then
                        ...
                        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Character Deleted', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** deleted ' .. result[1].name .. '\'s character ' .. charInfo.firstname .. ' ' .. charInfo.lastname, false)
                    else
...
RegisterServerEvent('919-admin:server:ClearInventory', function(targetId)
    ...
    if AdminPanel.HasPermission(src, 'clearinventory') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Inventory Cleared', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** has cleared the inventory of ' .. GetPlayerName(targetId), false)
...
RegisterServerEvent('919-admin:server:SetJob', function(targetId, job, grade)
    ...
    if AdminPanel.HasPermission(src, 'setjob') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Set Job', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** set the job of ' .. GetPlayerName(targetId) .. ' to ' .. job .. ' (' .. grade .. ')', false)
...
RegisterServerEvent('919-admin:server:SetGang', function(targetId, gang, grade)
    ...
    if AdminPanel.HasPermission(src, 'setgang') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Set Gang', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** set the gang of ' .. GetPlayerName(targetId) .. ' to ' .. gang .. ' (' .. grade .. ')', false)
...
RegisterServerEvent('919-admin:server:GiveItem', function(targetId, item, amount)
    ...
    if AdminPanel.HasPermission(src, 'giveitem') then
        ...
        if success then
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Item Given', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** gave ' .. item .. ' (x' .. amount .. ') to ' .. GetPlayerName(targetId), false)
...
AdminPanel.GiveItemFromDiscord = function(targetId, item, amount)
    local success, errorMsg = handleItemAction('add', targetId, item, amount)
    if success then
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Item Given', 'red', '**DISCORD** gave ' .. item .. ' (x' .. amount .. ') to ' .. GetPlayerName(targetId), false)
        Bridge.Notify(targetId, Lang:t('notify.givenItem', { value = item }), 'success')
    end
...
AdminPanel.RemoveItemFromDiscord = function(targetId, item, amount)
    ...
    if success then
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Item Removed', 'red', '**DISCORD** removed ' .. item .. ' (x' .. amount .. ') from ' .. GetPlayerName(targetId), false)
...
RegisterServerEvent('919-admin:server:FireJob', function(targetId)
    ...
    if AdminPanel.HasPermission(src, 'firejob') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Set Job', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** fired ' .. GetPlayerName(targetId) .. ' from their job.', false)
...
RegisterServerEvent('919-admin:server:FireGang', function(targetId)
    ...
    if AdminPanel.HasPermission(src, 'firegang') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Fired From Gang', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** removed ' .. GetPlayerName(targetId) .. ' from their gang.', false)
...
RegisterServerEvent('919-admin:server:FireJobByCitizenId', function(citizenId)
    ...
    if AdminPanel.HasPermission(src, 'firejob') then
        if Config.Framework == 'qbox' then
            ...
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Set Job', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** removed ' .. citizenId .. ' from their job.', false)
            TriggerClientEvent('919-admin:client:ShowPanelAlert', src, 'success', '<strong>' .. Lang:t('alerts.success') .. '</strong> ' .. Lang:t('alerts.firedJob'))
            return
        end
        ...
        if targetPlayer then
            if Config.Framework == 'qbcore' then
                ...
                targetPlayer.Functions.Save()
                TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Set Job', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** removed ' .. GetPlayerName(targetPlayer.PlayerData.source) .. ' from their job.', false)
            elseif Config.Framework == 'esx' then
            ...
            if Config.Framework == 'qbcore' then
                ...
                MySQL.update('UPDATE `' .. Config.DB.CharactersTable .. '` SET `job` = ? WHERE `citizenid` = ?', { json.encode(PlayerData.job), citizenId }, function(rowsAffected)
                    if rowsAffected ~= 0 and rowsAffected ~= nil then
                        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Set Job', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** removed (OFFLINE) Citizen ID ' .. citizenId .. ' from their job.', false)
                        TriggerClientEvent('919-admin:client:ShowPanelAlert', src, 'success', '<strong>' .. Lang:t('alerts.success') .. '</strong> ' .. Lang:t('alerts.firedJob'))
                    end
                end)
            elseif Config.Framework == 'esx' then
...
RegisterServerEvent('919-admin:server:FireGangByCitizenId', function(citizenId)
    ...
    if AdminPanel.HasPermission(src, 'firegang') then
        if Config.Framework == 'qbox' then
            ...
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Set Job Grade', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** removed ' .. citizenId .. ' from their gang.', false)
            TriggerClientEvent('919-admin:client:ShowPanelAlert', src, 'success', '<strong>' .. Lang:t('alerts.success') .. '</strong> ' .. Lang:t('alerts.firedGang'))
            return
        end
        ...
        if Config.Framework == 'qbcore' then
            ...
            if targetPlayer then
                ...
                targetPlayer.Functions.Save()
                TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Set Job Grade', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** removed ' .. GetPlayerName(targetPlayer.PlayerData.source) .. ' from their gang.', false)
                TriggerClientEvent('919-admin:client:ShowPanelAlert', src, 'success', '<strong>' .. Lang:t('alerts.success') .. '</strong> ' .. Lang:t('alerts.firedGang'))
            else -- Player is offline, so we"re going to formulate the default JSON for unemployed and set it to the offline character
                PlayerData = {}
                PlayerData.gang = {}
                ...
                MySQL.update('UPDATE `' .. Config.DB.CharactersTable .. '` SET `gang` = ? WHERE `citizenid` = ?', { json.encode(PlayerData.gang), citizenId }, function(rowsAffected)
                    if rowsAffected ~= 0 and rowsAffected ~= nil then
                        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Fired From Gang', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** removed (OFFLINE) Citizen ID ' .. citizenId .. ' from their gang.', false)
                        TriggerClientEvent('919-admin:client:ShowPanelAlert', src, 'success', '<strong>' .. Lang:t('alerts.success') .. '</strong> ' .. Lang:t('alerts.firedGang'))
                    end
                end)
            end
...
RegisterServerEvent('919-admin:server:SetGangGradeByCitizenId', function(citizenId, grade)
    ...
    if AdminPanel.HasPermission(src, 'setgang') then
        if Config.Framework == 'qbox' then
            ...
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Set Gang Grade', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** set gang grade of ' .. citizenId .. ' to ' .. grade .. '.', false)
        ...
        if Config.Framework == 'qbcore' then
            ...
            if targetPlayer then
                if QBCore.Shared.Gangs[targetPlayer.PlayerData.gang.name].grades[grade] ~= nil then
                    ...
                    targetPlayer.Functions.Save()
                    TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Set Gang Grade', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** set gang grade of ' .. GetPlayerName(targetPlayer.PlayerData.source) .. ' to ' .. grade .. '.', false)
                    ...
                    if gangInfo.grade ~= nil then
                        if QBCore.Shared.Gangs[gangInfo.name].grades[grade] ~= nil then
                            ...
                            MySQL.update('UPDATE `' .. Config.DB.CharactersTable .. '` SET `gang` = ? WHERE `citizenid` = ?', { json.encode(gangInfo), citizenId }, function(rowsAffected)
                                if rowsAffected ~= 0 and rowsAffected ~= nil then
                                    TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Set Gang Grade', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** set citizen id ' .. citizenId .. ' to gang grade ' .. grade .. ' (OFFLINE)', false)
                                    TriggerClientEvent('919-admin:client:ShowPanelAlert', src, 'success', '<strong>' .. Lang:t('alerts.success') .. '</strong> ' .. Lang:t('alerts.gangGradeSet', { value = grade }))
                                else
...
RegisterServerEvent('919-admin:server:SetJobGradeByCitizenId', function(citizenId, grade)
    ...
    if AdminPanel.HasPermission(src, 'setjob') then
        if Config.Framework == 'qbox' then
            local playerData = exports.qbx_core:GetPlayerByCitizenId(citizenId) or exports.qbx_core:GetOfflinePlayer(citizenId)
            exports.qbx_core:SetJob(citizenId, playerData.job.name, grade)
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Set Job', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** set job grade of ' .. GetPlayerName(targetPlayer.PlayerData.source) .. ' to ' .. grade .. '.', false)
                ...
                if Config.Framework == 'qbcore' then
                    ...
                    if QBCore.Shared.Jobs[jobInfo.name].grades[grade] ~= nil then
                        ...
                        MySQL.update('UPDATE `' .. Config.DB.CharactersTable .. '` SET `job` = ? WHERE `citizenid` = ?', { json.encode(jobInfo), citizenId }, function(rowsAffected)
                            if rowsAffected ~= 0 and rowsAffected ~= nil then
                                TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Set Job Grade', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** set citizen id ' .. citizenId .. ' to job grade ' .. grade .. ' (OFFLINE)', false)
...
RegisterServerEvent('919-admin:server:BringPlayer', function(targetId)
    ...
    if AdminPanel.HasPermission(src, 'teleport') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Teleport', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** brought **' .. GetPlayerName(targetId) .. '** [' .. targetId .. '] to them', false)
    end
end)
...
RegisterServerEvent('919-admin:server:GotoPlayer', function(targetId)
    ...
    if AdminPanel.HasPermission(src, 'teleport') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Teleport', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** teleported to **' .. GetPlayerName(targetId) .. '** [' .. targetId .. ']', false)
    end
end)
...
RegisterServerEvent('919-admin:server:OpenSkinMenu', function(targetId)
    ...
    if AdminPanel.HasPermission(src, 'clothing') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Skin Menu', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** opened skin menu for ' .. GetPlayerName(targetId), false)
    end
end)