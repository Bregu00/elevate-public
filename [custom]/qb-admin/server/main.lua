...
RegisterNetEvent('919-admin:server:RequestNoClip', function()
    ...
    if AdminPanel.HasPermission(src, 'noclip') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Admin ', 'red', SenderName .. ' Has toggled noclip: ' .. ' At: ' .. SentTime, false)
    end
end)
...
RegisterNetEvent('919-admin:server:AdminChatSend', function(message)
    ...
    if AdminPanel.HasPermission(src, 'adminchat') then
        ...
        TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Admin chat send', 'red', SenderName .. ' Has sent the following message in the adminchat: ' .. message .. ' At: ' .. SentTime, false)
...
RegisterNetEvent('919-admin:server:SendReport', function(subject, info, type)
    ...
    TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', Lang:t('alerts.reportSent'), 'red', sendername .. '' .. Lang:t('alerts.sentFollowingReport') .. ' ' .. subject .. ' ' .. Lang:t('alerts.message') .. ' ' .. info, false)
    for k, v in pairs(GetPlayers()) do
        v = tonumber(v)
...
RegisterServerEvent('919-admin:server:ClaimReport', function(id)
    ...
    if AdminPanel.HasPermission(src, 'claimreport') then
        if AdminPanel.Reports[tostring(id)] then
            ...
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', Lang:t('alerts.reportClaimed'), 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** claimed report ID ' .. id, false)
            for k, v in pairs(GetPlayers()) do
                v = tonumber(v)
...
RegisterServerEvent('919-admin:server:DeleteReport', function(id)
    ...
    if AdminPanel.HasPermission(src, 'deletereport') then
        if AdminPanel.Reports[tostring(id)] then
            ...
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Report Deleted', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** deleted report ID ' .. id, false)
...
RegisterNetEvent('919-admin:server:MonetaryAction', function(targetId, action, amount)
    ...
    if AdminPanel.HasPermission(src, 'givetakemoney') then
        if action == 'givecash' then
            ...
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Cash Given', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** added $' .. amount .. ' cash to ' .. GetPlayerName(targetId), false)
        elseif action == 'removecash' then
            ...
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Cash Removed', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** removed $' .. amount .. ' cash from ' .. GetPlayerName(targetId), false)
        elseif action == 'givebank' then
            ...
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Bank Given', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** added $' .. amount .. ' bank money to ' .. GetPlayerName(targetId), false)
        elseif action == 'removebank' then
            ...
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Bank Removed', 'red', '**STAFF MEMBER ' .. GetPlayerName(src) .. '** removed $' .. amount .. ' bank money from ' .. GetPlayerName(targetId), false)
        end
    end
end)
...
RegisterServerEvent('qb-log:server:CreateLog', function(source, name, title, color, message, tagEveryone)
    -- print(name, title, color, message, tagEveryone)
        exports.onl_logsender:SendLog(source, message, {
            ...
            discordWebhook = "https://discord.com/api/webhooks/1349306669980586037/pruJtbcgxnN5PpfD-Q1TwfLPNIlOeFpylRh2KS6fMUMqZ33tmG51h-gylmfJg3XtQq1D?thread_id=1349306180400320523" -- Another webhook
        })
...
AdminPanel.CreateCallback('919-admin:server:HasPermission', function(source, cb, permission)
    local src = source
    if AdminPanel.HasPermission(src, permission) then
        if permission == 'playernames' then
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Player Names Toggled', 'red', '**STAFF MEMBER ' .. GetPlayerName(source) .. '** has toggled their player names.', false)
        elseif permission == 'playerblips' then
            TriggerEvent('qb-log:server:CreateLog', src, 'adminactions', 'Player Blips Toggled', 'red', '**STAFF MEMBER ' .. GetPlayerName(source) .. '** has toggled their player blips.', false)
        end
        return cb(true)
    end
end)
...
elseif Config.Framework == 'esx' then
    AdminPanel.CreateCallback('919-admin:server:HasPermissions', function(source, cb, group)
        local src = source
        local retval = false
        if Config.ESXUseAcePerms then
            if IsPlayerAceAllowed(src, group) then retval = true end
        else
...
AdminPanel.HasPermission = function(targetId, permName)
    ...
    for k, v in pairs(Config.Permissions) do
        ...
        elseif Config.Framework == 'esx' then
            if Config.ESXUseAcePerms then
                if IsPlayerAceAllowed(targetId, k) then
                    hasPerms = true
                    for _, action in pairs(v.AllowedActions) do if action == permName then return true end end
                end
            else
...
AdminPanel.HasPermissionEx = function(targetId, permName)
    for k, v in pairs(Config.Permissions) do
        ...
        elseif Config.Framework == 'esx' then
            if Config.ESXUseAcePerms then
                if IsPlayerAceAllowed(targetId, k) then for _, action in pairs(v.AllowedActions) do if action == permName then return true end end end
            else
                local Player = Bridge.GetPlayer(targetId)
...
AdminPanel.GetRole = function(targetId)
    for k, v in ipairs(Config.RoleOrder) do
        ...
        elseif Config.Framework == 'esx' then
            if Config.ESXUseAcePerms then
                if IsPlayerAceAllowed(targetId, v) then return v end
            else
                local Player = Bridge.GetPlayer(targetId)
