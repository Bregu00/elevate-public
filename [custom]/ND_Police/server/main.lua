...
RegisterNetEvent('ND_Police:gsrTest', function(target)
    ...
    if state.shot then
        return Bridge.notify(src, {
            type = 'success',
            description = 'Fandt krudtslam (Har affyret skud)'
        })
    end

    Bridge.notify(src, {
        type = 'error',
        description = 'Intet krudtslam fundet (Har ikke affyret skud)'
    })
end)
...
RegisterNetEvent("ND_Police:impoundVehicle", function(netId, impoundReclaimPrice)
    ...
    if not impoundReclaimPrice or impoundReclaimPrice > config.maxImpoundPrice or impoundReclaimPrice < config.minImpoundPrice then
        return Bridge.notify({
            type = "error",
            description = "Fejl, forkert indtastet."
        })
    end
    ...
    if not Bridge.hasJobs(src, config.policeGroups) then
        return Bridge.notify({
            type = "error",
            description = "Du har ikke tilladelse til dette."
        })
    end
    ...
    if not DoesEntityExist(vehicle) then
        return Bridge.notify({
            type = "error",
            description = "Fandt ikke et køretøj, prøv igen."
        })
    end
    ...
    if #(vehCoords-pedCoords) > 5 then
        return Bridge.notify({
            type = "error",
            description = "Du er alt for langt væk fra køretøjet."
        })
    end
