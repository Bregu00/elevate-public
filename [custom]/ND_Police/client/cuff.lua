...
local function setCuffed(enabled, angle, cuffType)
    ...
    CreateThread(function()
        lib.disableControls:Add(140, 141, 142, 25, 59, 21, 22)
    ...
    CreateThread(function()
        ...
        lib.disableControls:Remove(140, 141, 142, 25, 59, 21, 22)
...
local function canCuffPed(ped, cuffType)
    ...
    anim = handsAnim["huk"]
    if IsEntityPlayingAnim(ped, anim.dict, anim.name, 3) or cuffType == "cuffs" or cuffType == "zipties" then
        return true, true
    end
end

exports("canCuffPed", canCuffPed)
...
local function getTargetPed()
    ...
    local player = lib.getClosestPlayer(coords, 2, false)
...
local function HandCuffPlayer()
    ...
    if isCuffed then
        if not Player(GetPlayerServerId(NetworkGetPlayerIndexFromPed(entity))).state.cuffType == "cuffs" then return end
        if not exports.ox_inventory:Search("count", "handcuffkey") then return end
        uncuffPed(targetPed, "cuffs")
    else
...
local keybind = lib.addKeybind({
    name = 'Handcuffs',
    description = 'Giv eller fjern håndjern',
    defaultKey = '',
    onPressed = function(self)
        HandCuffPlayer()
    end,
...
exports.ox_target:addGlobalPlayer({
    ...
    },
    {
        ...
        onSelect = function(data)
            ...
            ExecuteCommand("do **Visiterer spiller**")
