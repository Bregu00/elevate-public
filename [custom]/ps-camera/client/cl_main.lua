    ...
    CreateThread(function()
        ...
        while camera and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == vehicle) do
            ...
            elseif IsControlJustPressed(1, 176) then
                ...
                HideHUDThisFrame()
                exports['screenshot-basic']:requestScreenshotUpload('https://api.fivemanage.com/api/image?apiKey=' .. uploadHookOrSecret, 'file', function(data)
...
function usePhoto(data)
    local item = lib.callback.await("ps-camera:server:getItemSlotData", false, data.slot)
    photo = not photo

    if photo then
        SetNuiFocus(true, true);
        SendNUIMessage({action = "openPhoto", image = item.metadata.ps_image, location = item.metadata.location})
