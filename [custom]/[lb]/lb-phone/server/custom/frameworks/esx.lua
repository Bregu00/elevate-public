...
function GetPlayerVehicles(source)
    ...
    for i = 1, #vehicles do
        ...
        local newCar = {
            plate = vehicle.plate,
            type = vehicle.type,
            location = vehicle.stored,
            model = vehicleMods.model,
            parked = vehicle.parked == 1 or false,
            coords = vehicle.coords and json.decode(vehicle.coords) or nil
...
if ESX.RegisterCommand then
    ESX.RegisterCommand("toggleverified", "god", function(xPlayer, args, showError)
    ...
    ESX.RegisterCommand("changepassword", "god", function(xPlayer, args, showError)
    ...
    ESX.RegisterCommand("resetphonesecurity", "god", function(xPlayer, args, showError)
        local id = args.id
