...
local function updateBeard(playerPed, beardSlot)
	local Gender = IsMpPed(playerPed)
	if not Gender then lib.notify({ title = "Fejl", description = "Denne funktion er ikke lavet til din PED", icon = 'ban', iconColor = '#C53030' }) return end
    Drawables["Mask"].Table[Gender] = beardSlot
end

local function saveBeard()
    local playerPed = PlayerPedId()
    local maskDrawable = GetPedDrawableVariation(playerPed, 1)

    if GetResourceKvpInt('beardSlot') == maskDrawable then lib.notify({ title = 'Du har allerede gemt dette skæg', type = 'error' }) return end

    local alert = lib.alertDialog({
        header = 'Gem skægtype.',
        content = 'Vil du gemme: [' .. maskDrawable .. '] som din skægtype?',
        centered = true,
        cancel = true
    })
    if alert ~= 'confirm' then return end

    SetResourceKvpInt('beardSlot', maskDrawable)
	updateBeard(playerPed, maskDrawable)
    lib.notify({ title = 'Succes', description = 'Du har gemt denne skægtype', type = 'success' })
end	

RegisterNetEvent("esx:playerLoaded", function()
	Wait(1000)
    local playerPed = PlayerPedId()
    local maskDrawable = GetResourceKvpInt('beardSlot') or 0
	updateBeard(playerPed, maskDrawable)
end)

RegisterCommand("gemskæg", saveBeard)
		...
		for k,v in pairs(Table) do
			if not Toggle.Remember then
				...
				if not LastEquipped[which] then
					if k == Cur.Drawable then
						PlayToggleEmote(Toggle.Emote, function() LastEquipped[which] = Cur SetPedComponentVariation(Ped, Toggle.Drawable, v, Cur.Texture, 0) end)
						return true
					end
				else
					local Last = LastEquipped[which]
					PlayToggleEmote(Toggle.Emote, function() SetPedComponentVariation(Ped, Toggle.Drawable, Last.Drawable, Last.Texture, 0) LastEquipped[which] = false end)
					return true
				end
			end
		end
