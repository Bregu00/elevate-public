...
local function stateBagWrapper(keyFilter, cb)
    return AddStateBagChangeHandler(keyFilter, '', function(bagName, _, value, _, replicated)
        ...
        end, 'Timeout while waiting for entity to exist', 2000)
...
stateBagWrapper('sirenMode', function(veh, soundMode)
    ...
    if soundMode == 1 then
        PlaySoundFromEntity(soundId, 'WMSIRENS_SIREN_ALPHA_S', veh, 0, false, 0)
    elseif soundMode == 2 then
        PlaySoundFromEntity(soundId, 'WMSIRENS_SIREN_BRAVO_S', veh, 0, false, 0)
    elseif soundMode == 3 then
        PlaySoundFromEntity(soundId, 'WMSIRENS_SIREN_CHARLIE_D', veh, 0, false, 0)
    elseif soundMode == 4 then
        PlaySoundFromEntity(soundId, 'WMSIRENS_SIREN_FOXTROT_D', veh, 0, false, 0)
    end
end)
