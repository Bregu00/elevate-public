lib.callback.register('dunski-bet:getBalance', function(source)
...
lib.callback.register('dunski-bet:getBets', function(source)
  local xPlayer = ESX.GetPlayerFromId(source)
...
lib.callback.register('dunski-bet:getStats', function(source)
  local xPlayer = ESX.GetPlayerFromId(source)
  if xPlayer then
...
lib.callback.register('dunski-bet:getDepositLimitInfo', function(source)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  local result = MySQL.query.await('SELECT SUM(amount) as total FROM dunski_deposits WHERE player_identifier = ? AND deposit_date = ?', 
    { identifier, today })
...
lib.callback.register('dunski-bet:isAccountFrozen', function(source)
  local xPlayer = ESX.GetPlayerFromId(source)
...
lib.callback.register('dunski-bet:setAccountFrozen', function(source, targetId, frozen)
  local xPlayer = ESX.GetPlayerFromId(source)
...
lib.callback.register('dunski-bet:placeBet', function(source, gameId, team, betType, odds, amount)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  local event = MySQL.query.await('SELECT * FROM dunski_events WHERE id = ?', { gameId })
  ...
  local existingBets = MySQL.query.await('SELECT * FROM dunski_bets WHERE player_identifier = ? AND status = "active"', { identifier })
...
lib.callback.register('dunski-bet:cashoutBet', function(source, betId)
  local xPlayer = ESX.GetPlayerFromId(source)
  if not xPlayer then return false, "Spiller ikke fundet", 0, {} end
...
lib.callback.register('dunski-bet:getProfileData', function(source)
  local xPlayer = ESX.GetPlayerFromId(source)
  if not xPlayer then return nil end
  
  local identifier = xPlayer.getIdentifier()
  
  local betsResult = MySQL.query.await('SELECT COUNT(*) as total, SUM(CASE WHEN status = "won" THEN 1 ELSE 0 END) as won, SUM(CASE WHEN status = "lost" THEN 1 ELSE 0 END) as lost FROM dunski_bets WHERE player_identifier = ?', { identifier })
  ...
  local today = os.date("%Y-%m-%d")
  local depositResult = MySQL.query.await('SELECT SUM(amount) as total FROM dunski_deposits WHERE player_identifier = ? AND deposit_date = ?', { identifier, today })
...
lib.callback.register('dunski-bet:getPlayerJob', function(source)
  local xPlayer = ESX.GetPlayerFromId(source)
  if not xPlayer then return nil end
...
lib.callback.register('dunski-bet:getPlayerInfo', function(source, targetId)
  local xPlayer = ESX.GetPlayerFromId(source)
  local xTarget = ESX.GetPlayerFromId(targetId)
  ...
  local today = os.date("%Y-%m-%d")
  local depositResult = MySQL.query.await('SELECT SUM(amount) as total FROM dunski_deposits WHERE player_identifier = ? AND deposit_date = ?', { targetIdentifier, today })
...
lib.callback.register('dunski-bet:adminDepositMoney', function(source, targetId, amount, betalingsmetode)
  local xPlayer = ESX.GetPlayerFromId(source)
  local xTarget = ESX.GetPlayerFromId(targetId)
  ...
  exports['elevate-multijob']:addAccountBalance(xPlayer.getJob().name, originalAmount)
  exports['fh_bossmenu']:AddIncome(xPlayer.getJob().name, { amount = originalAmount, type = "income" }, xPlayer.getName())
  ...
  local approved = lib.callback.await('dunski-bet:approveAdminDeposit', targetId, { 
...
lib.callback.register('dunski-bet:adminWithdrawMoney', function(source, targetId, amount)
  local xPlayer = ESX.GetPlayerFromId(source)
  local xTarget = ESX.GetPlayerFromId(targetId)
  ...
  local totalPlayed = MySQL.query.await('SELECT SUM(amount) as total FROM dunski_bets WHERE player_identifier = ?', { targetIdentifier })
  ...
  if exports['elevate-multijob']:getAccountBalance(xPlayer.getJob().name) < amount then
    DiscordLog("admin", source, "Forsøgte at hæve " .. amount .. " DKK fra " .. xTarget.getName() .. ", men firmaet har ikke nok penge (Saldo: " .. currentBalance .. " DKK)")
  ...
  if updatePlayerBettingBalance(targetIdentifier, newBalance) then
    exports['elevate-multijob']:removeAccountBalance(xPlayer.getJob().name, amount)
    exports['fh_bossmenu']:AddIncome(xPlayer.getJob().name, { amount = amount, type = "expense" }, xPlayer.getName())
...
lib.callback.register('dunski-bet:getBettingEvents', function(source)
...
lib.callback.register('dunski-bet:createBettingEvent', function(source, eventData)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  local result = MySQL.insert.await('INSERT INTO dunski_events (sport_key, home_team, away_team, home_odds, draw_odds, away_odds, home_color, away_color, commence_time, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', {
    eventData.sport_key,
    eventData.home_team,
    eventData.away_team,
    eventData.home_odds,
  ...
  if result then
    ...
    TriggerClientEvent('dunski-bet:refreshBettingData', -1)
...
lib.callback.register('dunski-bet:updateEventOdds', function(source, eventId, homeOdds, drawOdds, awayOdds)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  local event = MySQL.query.await('SELECT * FROM dunski_events WHERE id = ?', { eventId })
  if not event or not event[1] then
    DiscordLog("events", source, "Forsøgte at opdatere ikke-eksisterende begivenhed #" .. eventId)
    return false, "Begivenhed ikke fundet"
  end
  
  local result = MySQL.update.await('UPDATE dunski_events SET home_odds = ?, draw_odds = ?, away_odds = ? WHERE id = ?', {
    homeOdds,
    drawOdds,
    awayOdds,
    eventId
...
lib.callback.register('dunski-bet:toggleEventStatus', function(source, eventId, isActive)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  local result = MySQL.update.await('UPDATE dunski_events SET is_active = ? WHERE id = ?', {
    isActive,
    eventId
  })
...
lib.callback.register('dunski-bet:setEventWinner', function(source, eventId, winner)
  local xPlayer = ESX.GetPlayerFromId(source)
  
  local result = MySQL.update.await('UPDATE dunski_events SET is_finished = 1, is_active = 0, winner = ? WHERE id = ?', {
    winner,
    eventId
  })
  ...
  if result and result > 0 then
    ...
    if event and event[1] then
      local bets = MySQL.query.await('SELECT * FROM dunski_bets WHERE status = "active"')
      ...
      if winner ~= "cancelled" then
        ...
        if hasWinners then
          ...
          -- print("^3No winners found for this event, skipping private announcement^7")
        end
      end
    end
...
lib.callback.register('dunski-bet:deleteEvent', function(source, eventId)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  local result = MySQL.update.await('DELETE FROM dunski_events WHERE id = ?', { eventId })
...
lib.callback.register('dunski-bet:getAllBets', function(source)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  local bets = MySQL.query.await('SELECT * FROM dunski_bets ORDER BY id DESC')
  ...
  if #duplicatesToDelete > 0 then
    ...
    MySQL.query.await("DELETE FROM dunski_bets WHERE id IN (" .. table.concat(placeholders, ",") .. ")", duplicatesToDelete)
...
lib.callback.register('dunski-bet:cancelBet', function(source, betId)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  local bet = MySQL.query.await('SELECT * FROM dunski_bets WHERE id = ?', { betId })
...
lib.callback.register('dunski-bet:getPlayerIdentifier', function(source, targetId)
  local xPlayer = ESX.GetPlayerFromId(source)
  local xTarget = ESX.GetPlayerFromId(targetId)
...
lib.callback.register('dunski-bet:getPlayerIdFromIdentifier', function(source, identifier)
  local xPlayer = ESX.GetPlayerFromId(source)
...
lib.callback.register('dunski-bet:getPlayerDetailedData', function(source, playerIdentifier)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  local bets = MySQL.query.await('SELECT * FROM dunski_bets WHERE player_identifier = ? ORDER BY id DESC', { playerIdentifier })
  ...
  local betsResult = MySQL.query.await('SELECT COUNT(*) as total, SUM(CASE WHEN status = "won" OR status = "cashed_out" THEN 1 ELSE 0 END) as won, SUM(CASE WHEN status = "lost" THEN 1 ELSE 0 END) as lost FROM dunski_bets WHERE player_identifier = ?', { playerIdentifier })
  ...
  local avgBetResult = MySQL.query.await('SELECT AVG(amount) as avg_amount FROM dunski_bets WHERE player_identifier = ?', { playerIdentifier })
  ...
  local biggestWinResult = MySQL.query.await('SELECT MAX(potential_win) as max_win FROM dunski_bets WHERE player_identifier = ? AND (status = "won" OR status = "cashed_out")', { playerIdentifier })
  local biggestWin = 0
...
lib.callback.register('dunski-bet:changeBetStatus', function(source, betId, newStatus)
  local xPlayer = ESX.GetPlayerFromId(source)
...
lib.callback.register('dunski-bet:placeParlay', function(source, selections, amount)
  local xPlayer = ESX.GetPlayerFromId(source)
  if not xPlayer then return false, "Spiller ikke fundet", 0 end
  ...
  for _, selection in ipairs(selections) do
    ...
    local event = MySQL.query.await('SELECT * FROM dunski_events WHERE id = ?', { selection.gameId })
    if not event or not event[1] then
...
lib.callback.register('dunski-bet:getEconomicOverview', function(source)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  local revenueResult = MySQL.query.await('SELECT SUM(amount) as total FROM dunski_bets')
  local totalRevenue = revenueResult and revenueResult[1] and revenueResult[1].total or 0
  
  local payoutResult = MySQL.query.await('SELECT SUM(potential_win) as total FROM dunski_bets WHERE status = "cashed_out"')
  local totalPayout = payoutResult and payoutResult[1] and payoutResult[1].total or 0
  
  local netProfit = totalRevenue - totalPayout
  
  local activeBetsResult = MySQL.query.await('SELECT SUM(amount) as total FROM dunski_bets WHERE status = "active"')
  local activeBetsValue = activeBetsResult and activeBetsResult[1] and activeBetsResult[1].total or 0
  
  local potentialPayoutResult = MySQL.query.await('SELECT SUM(potential_win) as total FROM dunski_bets WHERE status = "active"')
...
lib.callback.register('dunski-bet:getDailyStatistics', function(source)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  local betsPlacedResult = MySQL.query.await('SELECT COUNT(*) as count FROM dunski_bets WHERE DATE(created_at) = ?', {today})
  local betsPlaced = betsPlacedResult and betsPlacedResult[1] and betsPlacedResult[1].count or 0
  
  local depositsResult = MySQL.query.await('SELECT SUM(amount) as total FROM dunski_deposits WHERE deposit_date = ?', {today})
  local totalDeposited = depositsResult and depositsResult[1] and depositsResult[1].total or 0
  
  local withdrawalsResult = MySQL.query.await('SELECT SUM(potential_win) as total FROM dunski_bets WHERE status = "cashed_out" AND DATE(created_at) = ?', {today})
  ...
  local wonBetsResult = MySQL.query.await('SELECT COUNT(*) as count, SUM(potential_win) as total FROM dunski_bets WHERE (status = "won" OR status = "cashed_out") AND DATE(created_at) = ?', {today})
  ...
  local lostBetsResult = MySQL.query.await('SELECT COUNT(*) as count, SUM(amount) as total FROM dunski_bets WHERE status = "lost" AND DATE(created_at) = ?', {today})
  ...
  local refundedBetsResult = MySQL.query.await('SELECT COUNT(*) as count, SUM(amount) as total FROM dunski_bets WHERE status = "refunded" AND DATE(created_at) = ?', {today})
...
lib.callback.register('dunski-bet:getMonthlyStatistics', function(source)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  local betsPlacedResult = MySQL.query.await('SELECT COUNT(*) as count FROM dunski_bets WHERE DATE(created_at) BETWEEN ? AND ?', {startDate, endDate})
  local betsPlaced = betsPlacedResult and betsPlacedResult[1] and betsPlacedResult[1].count or 0
  
  local depositsResult = MySQL.query.await('SELECT SUM(amount) as total FROM dunski_deposits WHERE deposit_date BETWEEN ? AND ?', {startDate, endDate})
  local totalDeposited = depositsResult and depositsResult[1] and depositsResult[1].total or 0
  
  local withdrawalsResult = MySQL.query.await('SELECT SUM(potential_win) as total FROM dunski_bets WHERE status = "cashed_out" AND DATE(created_at) BETWEEN ? AND ?', {startDate, endDate})
  local totalWithdrawn = withdrawalsResult and withdrawalsResult[1] and withdrawalsResult[1].total or 0
  
  local netIncome = totalDeposited - totalWithdrawn
  
  local wonBetsResult = MySQL.query.await('SELECT COUNT(*) as count, SUM(potential_win) as total FROM dunski_bets WHERE (status = "won" OR status = "cashed_out") AND DATE(created_at) BETWEEN ? AND ?', {startDate, endDate})
  local wonBets = wonBetsResult and wonBetsResult[1] and wonBetsResult[1].count or 0
  local wonAmount = wonBetsResult and wonBetsResult[1] and wonBetsResult[1].total or 0
  
  local lostBetsResult = MySQL.query.await('SELECT COUNT(*) as count, SUM(amount) as total FROM dunski_bets WHERE status = "lost" AND DATE(created_at) BETWEEN ? AND ?', {startDate, endDate})
  local lostBets = lostBetsResult and lostBetsResult[1] and lostBetsResult[1].count or 0
  local lostAmount = lostBetsResult and lostBetsResult[1] and lostBetsResult[1].total or 0
  
  local refundedBetsResult = MySQL.query.await('SELECT COUNT(*) as count, SUM(amount) as total FROM dunski_bets WHERE status = "refunded" AND DATE(created_at) BETWEEN ? AND ?', {startDate, endDate})
  local refundedBets = refundedBetsResult and refundedBetsResult[1] and refundedBetsResult[1].count or 0
  local refundedAmount = refundedBetsResult and refundedBetsResult[1] and refundedBetsResult[1].total or 0
  ...
  for day = 1, daysInMonth do
    ...
    local dayBetsPlacedResult = MySQL.query.await('SELECT COUNT(*) as count FROM dunski_bets WHERE DATE(created_at) = ?', {dayDate})
    local dayBetsPlaced = dayBetsPlacedResult and dayBetsPlacedResult[1] and dayBetsPlacedResult[1].count or 0
    
    local dayDepositsResult = MySQL.query.await('SELECT SUM(amount) as total FROM dunski_deposits WHERE deposit_date = ?', {dayDate})
    local dayTotalDeposited = dayDepositsResult and dayDepositsResult[1] and dayDepositsResult[1].total or 0
    
    local dayWithdrawalsResult = MySQL.query.await('SELECT SUM(potential_win) as total FROM dunski_bets WHERE status = "cashed_out" AND DATE(created_at) = ?', {dayDate})
    ...
    local dayWonBetsResult = MySQL.query.await('SELECT COUNT(*) as count, SUM(potential_win) as total FROM dunski_bets WHERE (status = "won" OR status = "cashed_out") AND DATE(created_at) = ?', {dayDate})
    ...
    local dayLostBetsResult = MySQL.query.await('SELECT COUNT(*) as count, SUM(amount) as total FROM dunski_bets WHERE status = "lost" AND DATE(created_at) = ?', {dayDate})
...
lib.callback.register('dunski-bet:exportEconomicData', function(source)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  local withdrawalsResult = MySQL.query.await('SELECT SUM(potential_win) as total FROM dunski_bets WHERE status = "cashed_out" AND DATE(updated_at) BETWEEN ? AND ?', {startDate, endDate})
  local totalWithdrawn = withdrawalsResult and withdrawalsResult[1] and withdrawalsResult[1].total or 0
...
lib.callback.register('dunski-bet:getProfitLossAnalysis', function(source)
  local xPlayer = ESX.GetPlayerFromId(source)
  ...
  local sportProfitQuery = [[
    ...
    FROM 
      dunski_bets b
    JOIN 
      dunski_events e ON JSON_EXTRACT(b.selections, '$[0].event_id') = e.id
    WHERE 
...
lib.callback.register('dunski-bet:getTopPlayers', function(source)
  local xPlayer = ESX.GetPlayerFromId(source)
