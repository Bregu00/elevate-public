CreateThread(function()
  MySQL.query([[
    CREATE TABLE IF NOT EXISTS dunski_bets (
  ...
  MySQL.query([[
    CREATE TABLE IF NOT EXISTS dunski_balances (
  ...
  MySQL.query([[
    CREATE TABLE IF NOT EXISTS dunski_stats (
      player_identifier VARCHAR(50) PRIMARY KEY,
  ...
  MySQL.query([[
    CREATE TABLE IF NOT EXISTS dunski_deposits (
      id INT AUTO_INCREMENT PRIMARY KEY,
      player_identifier VARCHAR(50) NOT NULL,
      amount FLOAT NOT NULL,
  ...
  MySQL.query([[
    CREATE TABLE IF NOT EXISTS dunski_events (
      id INT AUTO_INCREMENT PRIMARY KEY,
  ...
  -- Check if home_color column exists, add if not
  MySQL.query([[
    SELECT COUNT(*) as count FROM information_schema.columns 
    WHERE table_schema = DATABASE() 
    AND table_name = 'dunski_events' 
    AND column_name = 'home_color'
  ]], {}, function(result)
    if result[1].count == 0 then
      MySQL.query([[
        ALTER TABLE dunski_events 
  ...
  -- Check if frozen column exists in dunski_balances, add if not
  MySQL.query([[
    SELECT COUNT(*) as count FROM information_schema.columns 
    WHERE table_schema = DATABASE() 
    AND table_name = 'dunski_balances' 
    AND column_name = 'frozen'
  ]], {}, function(result)
    if result[1].count == 0 then
      MySQL.query([[
        ALTER TABLE dunski_balances 
  ...
  -- DiscordLog("events", 0, "Database tabeller er blevet initialiseret")
end)

-- Player bets functions
function getPlayerBets(identifier)
  local result = MySQL.query.await('SELECT * FROM dunski_bets WHERE player_identifier = ? ORDER BY created_at DESC', { identifier })
  if result then
...
function getPlayerBettingBalance(identifier)
  local result = MySQL.query.await('SELECT balance FROM dunski_balances WHERE player_identifier = ?', { identifier })
  if result and result[1] then
    return result[1].balance
  else
    MySQL.insert.await('INSERT INTO dunski_balances (player_identifier, balance, frozen) VALUES (?, ?, ?)', { identifier, 0, false })
    return 0
  end
end

function updatePlayerBettingBalance(identifier, newBalance)
  local result = MySQL.update.await('UPDATE dunski_balances SET balance = ? WHERE player_identifier = ?', { newBalance, identifier })
  return result > 0
end

function isAccountFrozen(identifier)
  local result = MySQL.query.await('SELECT frozen FROM dunski_balances WHERE player_identifier = ?', { identifier })
  if result and result[1] then
    -- print("^3Frozen status for " .. identifier .. ": " .. tostring(result[1].frozen) .. " (Type: " .. type(result[1].frozen) .. ")^7")
...
function setAccountFrozen(identifier, frozen)
  ...
  local result = MySQL.update.await('UPDATE dunski_balances SET frozen = ? WHERE player_identifier = ?', { frozenValue, identifier })
  return result > 0
end

-- Stats functions
function getPlayerBettingStats(identifier)
  local result = MySQL.query.await('SELECT total_winnings, total_losses FROM dunski_stats WHERE player_identifier = ?', { identifier })
  if result and result[1] then
    return result[1]
  else
    MySQL.insert.await('INSERT INTO dunski_stats (player_identifier, total_winnings, total_losses) VALUES (?, ?, ?)', { identifier, 0, 0 })
    return { total_winnings = 0, total_losses = 0 }
  end
end

function updatePlayerBettingStats(identifier, totalWinnings, totalLosses)
  local result = MySQL.update.await('UPDATE dunski_stats SET total_winnings = ?, total_losses = ? WHERE player_identifier = ?', { totalWinnings, totalLosses, identifier })
  return result > 0
end

-- Bet functions
function addBet(identifier, amount, betType, selections, potentialWin)
  local betId = MySQL.insert.await('INSERT INTO dunski_bets (player_identifier, amount, bet_type, status, selections, potential_win) VALUES (?, ?, ?, ?, ?, ?)', {
    identifier,
    amount,
    betType,
    'active',
...
function updateBetStatus(betId, status)
  local result = MySQL.update.await('UPDATE dunski_bets SET status = ? WHERE id = ?', { status, betId })
  return result > 0
end
...
function recordDeposit(identifier, amount)
  ...
  local result = MySQL.insert.await('INSERT INTO dunski_deposits (player_identifier, amount, deposit_date) VALUES (?, ?, ?)',
...
function getBettingEvents(activeOnly)
  local query = 'SELECT * FROM dunski_events WHERE winner IS NULL'
  if activeOnly then
  ...
  if result then
    for _, event in ipairs(result) do
      ...
      local eventTime = math.floor(event.commence_time / 1000) -- Convert milliseconds to seconds
...
function formatBettingEvent(event)
  ...
  if dbTimestamp then
    ...
    elseif type(dbTimestamp) == "number" then
      eventTime = math.floor(dbTimestamp / 1000) -- Convert milliseconds to seconds
    end
...
function updateParlayStatus(betId)
  ...
  for _, selection in ipairs(selections) do
      if selection.event_id then
          local event = MySQL.query.await('SELECT * FROM dunski_events WHERE id = ?', { selection.event_id })
