...
RegisterNetEvent('dunski-bet:receiveBettingData')
AddEventHandler('dunski-bet:receiveBettingData', function(data)
...
RegisterNUICallback("FetchBets", function(data, cb)
  ...
  TriggerServerEvent('dunski-bet:fetchBettingData')
  playerBets = lib.callback.await('dunski-bet:getBets', false)
...
RegisterNUICallback("FetchBalance", function(data, cb)
  playerBalance = lib.callback.await('dunski-bet:getBalance', false)
  cb(playerBalance)
end)
...
RegisterNUICallback("FetchStats", function(data, cb)
  local stats = lib.callback.await('dunski-bet:getStats', false)
  cb(stats)
end)
...
RegisterNUICallback("PlaceBet", function(data, cb)
  ...
  local isFrozen = lib.callback.await('dunski-bet:isAccountFrozen', false)
  if isFrozen then
    exports["lb-phone"]:SendNotification({
      app = Config.Identifier,
      title = Config.AppName,
  ...
  local success, message, newBalance = lib.callback.await('dunski-bet:placeBet', false, 
...
RegisterNUICallback("CashoutBet", function(data, cb)
  ...
  local success, message, newBalance, newBets = lib.callback.await('dunski-bet:cashoutBet', false, data.betId)
...
RegisterNUICallback("FetchProfileData", function(data, cb)
  local profileData = lib.callback.await('dunski-bet:getProfileData', false)
  cb(profileData)
end)
...
RegisterNUICallback("PlaceParlay", function(data, cb)
  ...
  local success, message, newBalance = lib.callback.await('dunski-bet:placeParlay', false, selections, inputAmount)
