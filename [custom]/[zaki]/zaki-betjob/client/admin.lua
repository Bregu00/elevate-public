function RegisterBettingAdminMenus()
  lib.registerContext({
    id = 'dunski_bet_admin_main',
    title = 'DUNSKI BET Administration',
    options = {
      {
...
function PromptAndHandlePlayerById()
  ...
  if input then
    ...
    if targetId and targetId > 0 then
      local success, playerInfo = lib.callback.await('dunski-bet:getPlayerInfo', false, targetId)
...
function CreateNewBettingEvent()
  ...
  if input then
    ...
    local success, message = lib.callback.await('dunski-bet:createBettingEvent', false, {
      sport_key = sportKey,
      home_team = homeTeam,
      away_team = awayTeam,
      home_odds = homeOdds,
...
function LoadAndShowBettingEvents()
  local bettingEvents = lib.callback.await('dunski-bet:getBettingEvents', false)
  ...
  lib.registerContext({
    id = 'dunski_bet_events_list',
    title = 'Væddemål',
    menu = 'dunski_bet_admin_main',
    options = options
  })

  lib.showContext('dunski_bet_events_list')
end

function ShowEventManagementMenu(event)
  lib.registerContext({
    id = 'dunski_bet_event_management',
    title = event.home_team .. ' vs ' .. event.away_team,
    menu = 'dunski_bet_events_list',
    options = {
      {
        title = 'Rediger Odds',
  ...
  lib.showContext('dunski_bet_event_management')
end
...
function EditEventOdds(event)
  ...
  if input then
    ...
    local success, message = lib.callback.await('dunski-bet:updateEventOdds', false, event.id, homeOdds, drawOdds, awayOdds)
...
function ToggleEventStatus(event)
  local success, message = lib.callback.await('dunski-bet:toggleEventStatus', false, event.id, event.is_active == 0)
...
function SetEventWinner(event)
  ...
  if input then
    ...
    local success, message = lib.callback.await('dunski-bet:setEventWinner', false, event.id, winner)
...
function DeleteEvent(event)
  ...
  if confirm == 'confirm' then
    local success, message = lib.callback.await('dunski-bet:deleteEvent', false, event.id)
...
function LoadAndShowAllBets()
  local allBets = lib.callback.await('dunski-bet:getAllBets', false)
  ...
  lib.registerContext({
    id = 'dunski_bet_all_bets_list',
    title = 'Alle Væddemål',
    menu = 'dunski_bet_admin_main',
    options = options
  })
  
  lib.showContext('dunski_bet_all_bets_list')
end
...
function ShowBetDetailsMenu(bet)
  ...
  lib.registerContext({
    id = 'dunski_bet_details',
    title = 'Væddemål #' .. bet.id,
    menu = 'dunski_bet_all_bets_list',
    options = options
  })
  
  lib.showContext('dunski_bet_details')
end
...
function ChangeBetStatus(bet, newStatus)
  ...
  if confirm == 'confirm' then
    local success, message = lib.callback.await('dunski-bet:changeBetStatus', false, bet.id, newStatus)
...
function CancelBet(bet)
  ...
  if confirm == 'confirm' then
    local success, message = lib.callback.await('dunski-bet:cancelBet', false, bet.id)
...
function RegisterPlayerManagementMenus(targetId, playerInfo)
  ...
  table.insert(options, {
    title = playerInfo.isFrozen and 'EFUS Status: Medlem' or 'EFUS Status: Ikke medlem!',
  ...
  table.insert(options, {
    title = playerInfo.isFrozen and 'Afmeld EFUS' or 'Tilmeld EFUS',
    ...
    onSelect = function()
      ...
      if confirm == 'confirm' then
        local success, message = lib.callback.await('dunski-bet:setAccountFrozen', false, targetId, not playerInfo.isFrozen)
        ...
        if success then
          Wait(500)
          local newSuccess, newPlayerInfo = lib.callback.await('dunski-bet:getPlayerInfo', false, targetId)
          if newSuccess then
      ...
      onSelect = function()
        ...
        if input then
          ...
          if amount and amount > 0 then
            local success, message = lib.callback.await('dunski-bet:adminDepositMoney', false, targetId, amount, betalingsmetode)
      ...
      onSelect = function()
        ...
        if input then
          ...
          if amount and amount > 0 and amount <= playerInfo.balance then
            local success, message = lib.callback.await('dunski-bet:adminWithdrawMoney', false, targetId, amount)
  ...
  lib.registerContext({
    id = 'dunski_bet_manage_player',
    title = 'Håndter Betting Konto',
    options = options
  })

  lib.showContext('dunski_bet_manage_player')
end


function ShowPlayerDetailedData(targetId, playerInfo)
  local playerIdentifier = lib.callback.await('dunski-bet:getPlayerIdentifier', false, targetId)
  ...
  local playerData = lib.callback.await('dunski-bet:getPlayerDetailedData', false, playerIdentifier)
  ...
  lib.registerContext({
    id = 'dunski_bet_player_stats',
    title = 'Statistik for ' .. playerInfo.name,
    menu = 'dunski_bet_manage_player',
    options = {
      {
  ...
  lib.showContext('dunski_bet_player_stats')
end
...
function ShowPlayerBetHistory(targetId, playerInfo, bets)
  ...
  lib.registerContext({
    id = 'dunski_bet_player_history',
    title = 'Væddemålshistorik',
    menu = 'dunski_bet_player_stats',
    options = options
  })
  
  lib.showContext('dunski_bet_player_history')
end
...
function ShowBetDetailsForPlayer(bet, playerInfo)
  ...
  lib.registerContext({
    id = 'dunski_bet_player_bet_details',
    title = 'Væddemål #' .. bet.id,
    menu = 'dunski_bet_player_history',
    options = options
  })
  
  lib.showContext('dunski_bet_player_bet_details')
end
...
function CancelPlayerBet(bet, playerInfo)
  ...
  if confirm == 'confirm' then
    ...
    if success then
      ...
      local targetId = lib.callback.await('dunski-bet:getPlayerIdFromIdentifier', false, bet.player_identifier)
      if targetId then
        local success, playerInfo = lib.callback.await('dunski-bet:getPlayerInfo', false, targetId)
        if success then
          ShowPlayerDetailedData(targetId, playerInfo)
...
lib.callback.register('dunski-bet:approveAdminDeposit', function(data)
    ...
    options = {
      {
        ...
        onSelect = function()
          local playerJob = lib.callback.await('dunski-bet:getPlayerJob', false)
          ...
          lib.showContext('dunski_bet_admin_main')
        end
      }
    }
  })
...
function ShowEconomicOverview()
  local economicData = lib.callback.await('dunski-bet:getEconomicOverview', false)
  ...
  lib.registerContext({
    id = 'dunski_bet_economic_overview',
    title = 'Økonomisk Oversigt',
    menu = 'dunski_bet_admin_main',
    options = options
  })
  
  lib.showContext('dunski_bet_economic_overview')
end

-- Function to show daily statistics
function ShowDailyStatistics()
  local dailyStats = lib.callback.await('dunski-bet:getDailyStatistics', false)
  ...
  lib.registerContext({
    id = 'dunski_bet_daily_statistics',
    title = 'Daglig Statistik',
    menu = 'dunski_bet_economic_overview',
    options = options
  })
  
  lib.showContext('dunski_bet_daily_statistics')
end

-- Function to show monthly statistics
function ShowMonthlyStatistics()
  local monthlyStats = lib.callback.await('dunski-bet:getMonthlyStatistics', false)
  ...
  lib.registerContext({
    id = 'dunski_bet_monthly_statistics',
    title = 'Månedlig Statistik',
    menu = 'dunski_bet_economic_overview',
    options = options
  })
  
  lib.showContext('dunski_bet_monthly_statistics')
end
...
function ShowMonthlyBreakdown(monthlyStats)
  ...
  lib.registerContext({
    id = 'dunski_bet_monthly_breakdown',
    title = 'Daglig Fordeling',
    menu = 'dunski_bet_monthly_statistics',
    options = options
  })
  
  lib.showContext('dunski_bet_monthly_breakdown')
end

-- Function to show profit/loss analysis
function ShowProfitLossAnalysis()
  local profitLossData = lib.callback.await('dunski-bet:getProfitLossAnalysis', false)
  ...
  lib.registerContext({
    id = 'dunski_bet_profit_loss_analysis',
    title = 'Profit/Tab Analyse',
    menu = 'dunski_bet_admin_main',
    options = options
  })
  
  lib.showContext('dunski_bet_profit_loss_analysis')
end
...
function ShowProfitBySport(sportData)
  ...
  lib.registerContext({
    id = 'dunski_bet_profit_by_sport',
    title = 'Profit pr. Sport',
    menu = 'dunski_bet_profit_loss_analysis',
    options = options
  })
  
  lib.showContext('dunski_bet_profit_by_sport')
end
...
function ShowProfitByDay(dayData)
  ...
  lib.registerContext({
    id = 'dunski_bet_profit_by_day',
    title = 'Profit pr. Ugedag',
    menu = 'dunski_bet_profit_loss_analysis',
    options = options
  })
  
  lib.showContext('dunski_bet_profit_by_day')
end
...
function ShowProfitByHour(hourData)
  ...
  lib.registerContext({
    id = 'dunski_bet_profit_by_hour',
    title = 'Profit pr. Time',
    menu = 'dunski_bet_profit_loss_analysis',
    options = options
  })
  
  lib.showContext('dunski_bet_profit_by_hour')
end
...
function ShowProfitByOdds(oddsData)
  ...
  lib.registerContext({
    id = 'dunski_bet_profit_by_odds',
    title = 'Profit pr. Odds Interval',
    menu = 'dunski_bet_profit_loss_analysis',
    options = options
  })
  
  lib.showContext('dunski_bet_profit_by_odds')
end

-- Function to show top players
function ShowTopPlayers()
  local topPlayersData = lib.callback.await('dunski-bet:getTopPlayers', false)
  ...
  lib.registerContext({
    id = 'dunski_bet_top_players',
    title = 'Top Spillere',
    menu = 'dunski_bet_admin_main',
    options = options
  })
  
  lib.showContext('dunski_bet_top_players')
end
...
function ShowMostActivePlayers(players)
  ...
  for i, player in ipairs(players) do
    table.insert(options, {
      ...
      onSelect = function()
        local targetId = lib.callback.await('dunski-bet:getPlayerIdFromIdentifier', false, player.identifier)
        if targetId then
          local success, playerInfo = lib.callback.await('dunski-bet:getPlayerInfo', false, targetId)
          if success then
            RegisterPlayerManagementMenus(targetId, playerInfo)
  ...
  lib.registerContext({
    id = 'dunski_bet_most_active_players',
    title = 'Mest Aktive Spillere',
    menu = 'dunski_bet_top_players',
    options = options
  })
  
  lib.showContext('dunski_bet_most_active_players')
end
...
function ShowBiggestWinners(players)
  ...
  lib.registerContext({
    id = 'dunski_bet_biggest_winners',
    title = 'Største Vindere',
    menu = 'dunski_bet_top_players',
    options = options
  })
  
  lib.showContext('dunski_bet_biggest_winners')
end
...
function ShowBiggestLosers(players)
  ...
  lib.registerContext({
    id = 'dunski_bet_biggest_losers',
    title = 'Største Tabere',
    menu = 'dunski_bet_top_players',
    options = options
  })
  
  lib.showContext('dunski_bet_biggest_losers')
end
...
function ShowHighestWinRate(players)
  ...
  lib.registerContext({
    id = 'dunski_bet_highest_win_rate',
    title = 'Højeste Vinderprocent',
    menu = 'dunski_bet_top_players',
    options = options
  })
  
  lib.showContext('dunski_bet_highest_win_rate')
end
...
function ShowBiggestSingleWins(wins)
  ...
  for i, win in ipairs(wins) do
    table.insert(options, {
      ...
      onSelect = function()
        local targetId = lib.callback.await('dunski-bet:getPlayerIdFromIdentifier', false, win.playerIdentifier)
        if targetId then
          local success, playerInfo = lib.callback.await('dunski-bet:getPlayerInfo', false, targetId)
          if success then
            RegisterPlayerManagementMenus(targetId, playerInfo)
  ...
  lib.registerContext({
    id = 'dunski_bet_biggest_single_wins',
    title = 'Største Enkelt Gevinster',
    menu = 'dunski_bet_top_players',
    options = options
  })
  
  lib.showContext('dunski_bet_biggest_single_wins')
end
