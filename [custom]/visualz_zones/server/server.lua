...
local getAllGangRanks = nil

CreateThread(function()
    exports["ap_createjob"]:getAllGangRanks(function(result)
        getAllGangRanks = result
    end)
    while getAllGangRanks == nil do Wait(1) end
    while true do
        Wait(1000)
...
function IsAllowedGang(gang)
    for _, v in pairs(getAllGangRanks) do
        if v.name == gang then
            return true
        end
    end
...
local function CheckControlReward(drugType, zone, xPlayer, popularDrugPrice)  
    if Config.PopularZoneDrugs[zone] and Config.PopularZoneDrugs[zone][drugType] then
...
function AddPoints(xPlayer, zone, drugPrice, drugType, amount)
    ...
    for k, v in pairs(Zones) do
        if k == zone then
            ...
            elseif (v.owner == gang and v.points >= 0) or (alliance and v.points >= 0) then
                ...
                TriggerClientEvent('ox_lib:notify', xPlayer.source, {
                    type = 'inform',
                    description = '+' .. (Config.PointsAddAmount * amount) .. ' Points i ' .. zoneName
                })
                hasGottenControlReward = true
                if CheckControlReward(drugType, zone, xPlayer, popularDrugPrice) then
                    TriggerClientEvent('ox_lib:notify', xPlayer.source, {
                ...
                v.points = v.points + (Config.PointsAddAmount * amount)
            elseif (v.owner ~= gang and not alliance and v.points >= 0) or (v.points < 0) then
                if v.points == 0 or v.points < 0 then
                    ...
                    v.points = v.points - (Config.PointsRemoveAmount * amount)
                    TriggerClientEvent('ox_lib:notify', xPlayer.source, {
                        type = 'inform',
                        description = '-' .. (Config.PointsRemoveAmount * amount) .. ' Points i ' .. zoneName
                    })
...
lib.callback.register("visualz_zones:SetPoint", function(source, zone, point)
    if not Zones[zone] then return false end
    if point > Config.MaximumPoints then return false end
    local point = tonumber(point)
    if point < 0 then return false end
...
lib.callback.register('visualz_zones:GetSpecificZone', function(source, zoneName)
