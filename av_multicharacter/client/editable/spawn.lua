RegisterNetEvent('av_multicharacter:defaultSpawn', function(isNew, data)
    if Config.Framework == "esx" then return end -- use esx:playerLoaded
    dbug('av_multicharacter:defaultSpawn')
    DoScreenFadeOut(500)
    Wait(2000)
    if Config.Framework == "qbox" then
        qboxStart(isNew, data)
        return
    end
...
AddEventHandler('esx:playerLoaded', function(playerData, isNew, skin)
    ...
    if isNew or not skin or #skin == 1 then
        ...
        Wait(1000)
        local randomSpawnCoords = Config.DefaultSpawns[math.random(1, #Config.DefaultSpawns)]
        dbug("SetEntityCoords(x,y,z,heading)", randomSpawnCoords.x, randomSpawnCoords.y, randomSpawnCoords.z, randomSpawnCoords.w)
        RequestCollisionAtCoord(randomSpawnCoords.x, randomSpawnCoords.y, randomSpawnCoords.z)
        Wait(500)
        SetEntityCoordsNoOffset(PlayerPedId(), randomSpawnCoords.x, randomSpawnCoords.y, randomSpawnCoords.z, true, true, false)
        SetEntityHeading(PlayerPedId(), randomSpawnCoords.w)
        Wait(2000)
