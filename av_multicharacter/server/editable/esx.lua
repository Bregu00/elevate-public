...
function GetPlayerSkin(identifier)
    ...
    if result then
        ...
        data = { model = GetHashKey(model), skin = skin }
    end
    return data
end
...
function DeleteCharacter(source, data)
    ...
    local checkQuery = 'SELECT 1 FROM %s WHERE %s = ? LIMIT 1'
    local queries = {}
    
    for table, column in pairs(Config.DeleteTables) do
        -- Check if the record exists
        local exists = MySQL.scalar.await(checkQuery:format(table, column), { identifier })
        if exists then
    ...
    if #queries > 0 then
        ...
        -- No records to delete, proceed to client event
        print(('[^2INFO^7] No records found for ^5%s^7, proceeding...'):format(identifier))
        Wait(50)
        TriggerClientEvent('av_multicharacter:init', src)
    end
end
