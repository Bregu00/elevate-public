...
function GetPlayerSkin(identifier)
    ...
    if result then
        local hash = result['model'] and tonumber(result['model']) or Config.DefaultPed
        dbug("Player Model ", hash)
        data = {model = hash, skin = result['skin']}
    end
    return data
end
...
function GetVehicleMods(source,identifier,plates)
    local data = false
