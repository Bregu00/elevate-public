...
Config.keepItemsOnRespawn = { "phone", "radio" } -- items that will not be removed when respawed (works only when Config.RemoveItemsOnRespawn is true)
...
Config.ambulanceStretchers = 1        -- how many stretchers should an ambulance have
...
function Config.giveVehicleKeys(vehicle, plate)
	local getVehicledisplayName = GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))

	exports['mani-keys']:SetJobKey(vehicle, 'ambulance')
