...
function utils.setupBedInteraction()
    local models = { `v_med_emptybed`, `v_med_bed1`, '-1519439119', '-289946279' }
    local options = {
        {
            name = "layOnBed",
            icon = "fas fa-bed",
            label = "Lig dig i sengen",
            onSelect = function(data)
                local playerPed = PlayerPedId()
                local bedCoords = GetEntityCoords(data.entity)
                local zOffset = 1.95

                if GetEntityModel(data.entity) == `v_med_emptybed` then
                    zOffset = 1.15
                end

                if GetEntityModel(data.entity) == `v_med_bed1` then
                    zOffset = 1.35
                end

                TaskStartScenarioAtPosition(playerPed, "WORLD_HUMAN_SUNBATHE_BACK", bedCoords.x, bedCoords.y, bedCoords.z + zOffset, GetEntityHeading(data.entity), 0, true, true)
                FreezeEntityPosition(playerPed, true)
                lib.showTextUI('[E] Rejs Dig', { alignIcon = 'center', icon = 'fa-solid fa-bed' })
