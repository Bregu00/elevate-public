...
local function getTreated(ped, hospital)

    local progBar = lib.progressBar({
        duration = 1500,
        label = 'Du bliver behandlet',
        useWhileDead = true,
        canCancel = true,
        disable = {
            car = true,
    ...
    local canBeRevived = lib.callback.await("visualz_cpr:server:CanBeRevived", false)
    if not canBeRevived then return utils.showNotification("Du er for tilskadekommed til at få hjælp") end
    ...
    lib.progressBar({ duration = 15000, label = locale("getting_treated"), useWhileDead = false, canCancel = false, disable = { car = true, move = true }, })
        ...
        Target.addLocalEntity(ped, {
            {
                ...
                fn = function()
                    if not allowAlwaysTreatment then
                        ...
                        if medicsOnline <= 5 then
                            getTreated(ped, hospital)
                        else
