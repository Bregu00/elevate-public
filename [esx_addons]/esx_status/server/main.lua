...
AddEventHandler('esx:playerDropped', function(playerId, reason)
    ...
    if not xPlayer then
        print('[ERROR] xPlayer not found for ID ' .. playerId)
        return
    end

    -- Get status from ESX.Players instead of an export
    ...
    if status and next(status) ~= nil then
        ...
        print('[WARNING] Status for player ' .. xPlayer.identifier .. ' was empty or nil on disconnect: ' .. tostring(status))
    end
