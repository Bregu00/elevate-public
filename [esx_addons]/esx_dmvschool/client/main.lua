...
CreateThread(function()
	local blip = AddBlipForCoord(Config.pedEntity.location.x, Config.pedEntity.location.y, Config.pedEntity.location.z)
...
CreateThread(function()
	local pedModel = GetHashKey(Config.pedEntity.pedModel)
	...
	local pedEntityLocation = Config.pedEntity.location
	while true do
		local sleep = 1500
		if CurrentTest == 'drive' then
			sleep = 0
			...
			if distance < 150 then
				if not DoesEntityExist(pedEntity) then
					...
					TaskStartScenarioInPlace(pedEntity, "WORLD_HUMAN_GUARD_STAND", 0, true)
					SetBlockingOfNonTemporaryEvents(pedEntity, true)
	
					exports.ox_target:addLocalEntity(pedEntity, {
						name = 'dmvschool', 
						label = "Osman's Køreskole",
