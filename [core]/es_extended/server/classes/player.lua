...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords, metadata, isAccountLocked)
    ---@class xPlayer
    local self = {}

    self.accounts = accounts
    self.coords = coords
    self.group = exports["fh_admin"]:getGroup(playerId)
    ...
    if Config.Multichar then
        ...
        if startIndex then
            self.license = ("steam%s"):format(identifier:sub(startIndex, identifier:len()))
        end
    else
        self.license = ("steam:%s"):format(identifier)
    end
    ...
    job.isgang = job.isgang or false

    self.isAccountLocked = isAccountLocked == 1 and true or false
    ...
    stateBag:set("isAccountLocked", self.isAccountLocked, true)
