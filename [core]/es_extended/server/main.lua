...
local loadPlayer = "SELECT `accounts`, `job`, `job_grade`, `group`, `position`, `inventory`, `skin`, `loadout`, `metadata`, `isAccountLocked`"
...
function loadESXPlayer(identifier, playerId, isNew)
    ...
    -- xPlayer Creation
    local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.loadout, GetPlayerName(playerId), userData.coords, userData.metadata, result.isAccountLocked)
    ...
    if result.firstname and result.firstname ~= "" then
        ...
        if result.isAccountLocked then
            xPlayer.set("isAccountLocked", result.isAccountLocked == 1 and true or false)
        end
