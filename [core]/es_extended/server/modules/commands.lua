ESX.RegisterCommand(
    ...
    function(xPlayer, args)
        ...
        TriggerEvent('qb-log:server:CreateLog', xPlayer.source, 'adminactions', 'Teleport', 'red', '**STAFF MEMBER ' .. GetPlayerName(xPlayer.source) .. '** teleported to ** ' .. args.x .. " " .. args.y .. " " ..  args.z, false)
...
ESX.RegisterCommand({ "clear", "cls" }, { "user", "mod", "admin", "god" }, function(xPlayer)
    xPlayer.triggerEvent("chat:clear")
end, false, { help = TranslateCap("command_clear") })

ESX.RegisterCommand({ "clearall", "clsall" }, "god", function(xPlayer)
...
ESX.RegisterCommand("refreshjobs", "god", function()
    ESX.RefreshJobs()
end, true, { help = TranslateCap("command_clearall") })
...
ESX.RegisterCommand("saveall", "god", function()
    Core.SavePlayers()
end, true, { help = TranslateCap("command_saveall") })

ESX.RegisterCommand("group", { "user", "mod", "admin", "god" }, function(xPlayer, _, _)
    print(("%s, you are currently: ^5%s^0"):format(xPlayer.getName(), xPlayer.getGroup()))
end, true)

ESX.RegisterCommand("job", { "user", "mod", "admin", "god" }, function(xPlayer, _, _)
...
ESX.RegisterCommand("info", { "user", "mod", "admin", "god" }, function(xPlayer)
    local job = xPlayer.getJob().name
    print(("^2ID: ^5%s^0 | ^2Name: ^5%s^0 | ^2Group: ^5%s^0 | ^2Job: ^5%s^0"):format(xPlayer.source, xPlayer.getName(), xPlayer.getGroup(), job))
end, false)

ESX.RegisterCommand("playtime", { "user", "mod", "admin", "god" }, function(xPlayer)
...
-- ESX.RegisterCommand("coords", "god", function(xPlayer)
--     local ped = GetPlayerPed(xPlayer.source)
...
ESX.RegisterCommand("tpm", {"mod", "admin", "god"}, function(xPlayer)
    xPlayer.triggerEvent("esx:tpm")
    TriggerEvent('qb-log:server:CreateLog', xPlayer.source, 'adminactions', 'Teleport', 'red', '**STAFF MEMBER ' .. xPlayer.name .. '** used teleport to marker **', false)
    if Config.AdminLogging then
...
ESX.RegisterCommand(
    ...
    function(xPlayer, args)
        ...
        local srcDim = GetPlayerRoutingBucket(tostring(xPlayer.source))
        local targetDim = GetPlayerRoutingBucket(args.playerId.source)

        if srcDim ~= targetDim then
            SetPlayerRoutingBucket(tostring(xPlayer.source), targetDim)
        end
        xPlayer.setCoords(targetCoords)
        TriggerEvent('qb-log:server:CreateLog', xPlayer.source, 'adminactions', 'Teleport', 'red', '**STAFF MEMBER ' .. GetPlayerName(xPlayer.source) .. '** teleported to **' .. GetPlayerName(args.playerId.source) .. '** [' .. args.playerId.source .. ']', false)
        if Config.AdminLogging then
...
ESX.RegisterCommand("noclip", "god", function(xPlayer)
...
ESX.RegisterCommand("players", "god", function()
