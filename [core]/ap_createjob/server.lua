...
ESX.RegisterServerCallback('ap_createjob:createJob',function(source, cb, jobName, jobLabel, jobWhitelisted, isGang)
	local xPlayer = ESX.GetPlayerFromId(source)
	local name = xPlayer.getName()
    if (jobName and jobLabel and jobWhitelisted) then
		MySQL.Async.fetchAll(
		'INSERT INTO jobs(name, label, whitelisted, isgang) VALUES (@jobName, @jobLabel, @jobWhitelisted, @jobisgang);',
		{
			['@jobName'] = jobName,
			['@jobLabel'] = jobLabel,
			...
			if result then	
				TriggerClientEvent('ap_createjob:notify', xPlayer.source, 'Successful')
				sendLogsDiscord(source, name, "Job name: "..jobName.." | Job label: "..jobLabel.." | Whitelisted status: ".. tostring(jobWhitelisted) .. " | isGang status: " .. tostring(isGang) .. " | Added to the database.")
				exports["esx_addonaccount"]:AddSharedAccount({name = "society_"..jobName, label = jobLabel}, 0)
				MySQL.Async.execute('INSERT INTO lunar_multijob_accounts (name, balance) VALUES (@jobName, 0);',{['@jobName'] = jobName,})
...
AddEventHandler("ap_createjob:RefreshJobs", function()
	...
	exports["elm_lab"]:refreshLabGangs()
	exports["elevate-multijob"]:getAllPlayerJobs()
	TriggerEvent("esx_addonaccount:refreshAccounts")
...
function getAllGangRanks(cb)
	MySQL.Async.fetchAll('SELECT * FROM jobs WHERE isgang = 1', {}, function(results)
		cb(results)
	end)
end

ESX.RegisterServerCallback('ap_createjob:server:getAllGangRanks',function(source, cb)
	getAllGangRanks(function(ranks)
		cb(ranks)
	end)
end)
